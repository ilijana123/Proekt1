...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Додајте event listener на "Shop" за приказ на категориите при mouseover
    shopButton.addEventListener('mouseover', function () {
        // Овде можете да ги прикажете категориите како што сакате
        // Пример:
        showCategories(['Clothes', 'Shoes', 'Fashion Accessories']);
    });

    // Функција за прикажување на категории
    function showCategories(categories) {
        // Очистете го содржината на континерот
        categoriesContainer.innerHTML = '';

        // Креирајте елементи за секоја категорија и додадете ги на континерот
        categories.forEach(category => {
            const categoryElement = document.createElement('div');
            categoryElement.textContent = category;

            // Додајте event listener за клик на секоја категорија
            categoryElement.addEventListener('click', function () {
                // Пренесете се на соодветната секција на страницата
                window.location.href = `#${category.toLowerCase()}-section`;
            });

            categoriesContainer.appendChild(categoryElement);
        });

        // Отворете/прикажете го континерот за категории
